...
public class FeedbackService : IFeedbackService
{
 ...
 "you know"
    ...
    private static string GetTopicCoverageNotes(string transcript, string[]? targetKeywords)
    {
        ...
        var matched = targetKeywords.Count(keyword => transcript.Contains(keyword, StringComparison.OrdinalIgnoreCase));
        return $"Matched {matched} of {targetKeywords.Length} target keywords.";
    }
    private static string GetImprovementSuggestion(int fillerWordCount, double wordsPerMinute, int wordCount, string transcript, string[]? targetKeywords)
    {
        ...
        {
            var matched = targetKeywords.Count(keyword => transcript.Contains(keyword, StringComparison.OrdinalIgnoreCase));
            if (matched < targetKeywords.Length) return "Try covering more of the prompt's expected points in your next answer.";
        }
